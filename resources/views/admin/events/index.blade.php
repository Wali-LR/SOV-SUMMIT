<x-admin-layout title="Events" subtitle="{{ $events->total() }} total">
    <x-slot name="actions">
    ...
    <div class="bg-white border border-slate-200 rounded-lg overflow-hidden">
        @if ($events->isEmpty())
            <div class="p-10 text-center">
                ...
                <a href="{{ route('admin.events.create') }}" class="inline-block mt-3 text-sm font-medium text-slate-900 underline">Create your first event</a>
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($events as $event)
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-3">
                                    @if ($event->cover_url)
                                        <img src="{{ $event->cover_url }}" alt="" class="h-10 w-14 rounded object-cover border border-slate-200">
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <div class="text-xs text-slate-500 font-mono">/{{ $event->slug }}</div>
                                </td>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-right">
                                    <a href="{{ route('events.show', $event) }}" target="_blank" class="text-slate-500 hover:text-slate-900 text-xs mr-3">View</a>
                                    <a href="{{ route('admin.events.edit', $event) }}" class="text-slate-900 hover:underline text-xs font-medium mr-3">Edit</a>
